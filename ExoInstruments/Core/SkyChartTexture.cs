    ...
    public static class SkyChartTexture
    {
        ...
        public static Color[] ComputePixels(List<SkyChartPoint> points, int width, int height, SkyChartView view, bool searchActive)
        {
            ...
            if (points != null)
            {
                ...
                // de-emphasized points first so emphasized (actual search-matched) points always render on top
                foreach (var p in points)
                    if (!ShouldEmphasize(p)) PlotStar(pixels, width, height, p, view, false);
                foreach (var p in points)
                    if (ShouldEmphasize(p)) PlotStar(pixels, width, height, p, view, true);
            }
