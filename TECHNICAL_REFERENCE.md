...
### 7.013 Adaptive optics (`VisualTelescopeSpec.AdaptiveOpticsFwhmArcsec`, `SolarSystemCameraTexture.ComputeSeeingBlurPx`)

Every ground-based instrument before SPHERE is seeing-limited: real atmospheric turbulence blurs the image to Paranal's typical ~0.6-1" regardless of aperture, modeled as the airmass-dependent pixel blur in §7.1. A real extreme-adaptive-optics system instead actively cancels that turbulence ahead of the wavefront sensor rather than just blurring the result, so it isn't modeled as a fixed pixel count: `AdaptiveOpticsFwhmArcsec` (nonzero only for SPHERE, §7.00) is a real achieved FWHM in arcseconds, converted to pixels at the *current* plate scale (`FWHM / PlateScaleArcsecPerPixel`), so it scales correctly with zoom/binning and replaces the plain airmass-based model entirely rather than adding to it. This is the single largest resolution gain in the whole roster: about 24-40× finer than FORS2's typical seeing-limited blur on the same 8.2m telescope, which is the entire reason SPHERE can resolve targets FORS2 can only show as a barely-resolved smudge.
...
- **Seeing blur**: for a plain (non-AO) instrument, grows linearly with `(airmass-1)`, capped at 6px. For SPHERE, replaced entirely by its own real adaptive-optics-corrected FWHM (§7.013) instead of the airmass-based model.
- **Defocus**: manual, only when autofocus is off, every instrument.
