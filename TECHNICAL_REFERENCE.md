...
| RC20 | 4 | 1.5 mm | **Estimated, not published.** Derivation below. |
| CDK1000 | 4 | 2.5 mm | **Estimated, not published.** Same method. |
...
#### 7.113.1 The two PlaneWave vane widths, which are estimates

**These two numbers are the only quantities in `VisualTelescopeCatalog` that no source backs.** They are recorded here in full, with the working, because a number nobody can trace is worth exactly as much as the account of how it was reached.

The problem is stated plainly: PlaneWave publishes no vane width for either the RC20 or the CDK1000, and neither do the retailers' spec sheets. Both telescopes visibly have a four-vane spider. The previous entry set `SpiderVaneCount = 0`, which drew **no spikes at all** — and a real telescope with no spikes is a larger error than a bracketed estimate, because a spike survives seeing: it extends over arcseconds, far beyond the seeing disc, so convolution broadens it without removing it. That is why amateur images of bright stars show spikes at all.

**Method A, rejected: equal pupil obscuration.** Assume a commercial spider removes the same fraction of the open pupil as a professional one. The two published spiders in this file give the bracket:

| | D | ε | w | vane run L | vanes / open pupil |
|---|---|---|---|---|---|
| HST OTA | 2.40 m | 0.330 | 26.4 mm | 0.804 m | 2.11 % |
| VLT UT1/UT3 | 8.20 m | 0.136 | 41.0 mm | 3.542 m | 1.12 % |

Geometric mean 1.54 %. Holding that fixed gives **4.3 mm** for the RC20 and **8.9 mm** for the CDK1000. Rejected: those are structural members, not blades, and no commercial OTA is built that way. The assumption fails because the vane run `L` is far shorter on a small telescope, so equal *area* fraction forces an absurd *thickness*.

**Method B, rejected: scale by the secondary the vane carries.** Physically better motivated — mass goes as `d₂³`, blade section as `w²` at fixed aspect ratio, so constant stress gives `w ∝ d₂^1.5`. Fitting the exponent to the two published points instead of assuming it gives `w = 35.6 mm · (d₂/1 m)^1.284`, and evaluates to **4.5 mm** (RC20) and **13.5 mm** (CDK1000). Rejected for the same reason and more strongly: extrapolating two orders of magnitude in secondary mass from a launch-qualified space structure and an 8 m alt-az to a carbon-fibre commercial tube is not a scaling, it is a hope.

**Method C, adopted: find out what actually sets the thickness.** Size the blade against the requirement it really has to meet, and see whether that requirement binds at all.

Secondary assembly mass from the published obstruction: `d₂ = ε·D`, fused silica at 2200 kg/m³, mirror thickness `d₂/6`, assembly (mirror + cell + baffle) taken at twice the mirror. Four vanes, the two aligned with the load carrying it axially, blade aspect `h/w = 25`, steel at `E = 200 GPa`, and a collimation budget of 25 µm of secondary decentre with the tube horizontal:

```
A = m·g·L / (2·E·δ)        w = sqrt(A / 25)
```

| | d₂ | mirror | assembly | L | section for 25 µm | w demanded |
|---|---|---|---|---|---|---|
| RC20 | 19.9 cm | 2.27 kg | ~4.5 kg | 15.6 cm | 0.69 mm² | **0.17 mm** |
| CDK1000 | 47.0 cm | 29.9 kg | ~59.8 kg | 26.5 cm | 15.5 mm² | **0.79 mm** |

**Stiffness does not bind.** It asks for tenths of a millimetre, which no one would build because such a blade would buckle under its own pretension and bend if a hand touched it. So the real driver is buckling, handling and stock availability, none of which is an optical quantity, and all of which land commercial blade spiders in the **1-3 mm** range.

Adopted: **1.5 mm** for the RC20 and **2.5 mm** for the CDK1000. Each sits above its own stiffness floor by a sensible margin (9× and 3×), and the ordering is right for the physics — the CDK1000's secondary is thirteen times heavier, so its blade is stouter, and by less of a margin.

**The cross-check that makes them coherent rather than merely plausible.** Feed them back through the obscuration these vanes produce:
...
Both land **below** the two professional spiders, by a factor of two to four. That is the expected direction and it was not imposed: a thin commercial blade should obscure less than a structural member sized for launch or for an 8 m mount. Method A got this backwards by construction, which is the clearest sign it was the wrong assumption.

**What the estimate is worth.** Spike brightness goes as the vane **area squared**, so a factor 1.7 spread in width (1.5 mm against the 2.5 mm one might equally argue for the RC20) is a factor 3 in spike brightness. Treat RC20 and CDK1000 spikes as correct in **direction, count and angular falloff**, all of which come from the pupil transform and not from this number, and as good to a factor of a few in **brightness**. Hubble, FORS2 and SPHERE are unaffected: their widths are published or derived from published masks.

**And it changes less on screen than it sounds.** Both PlaneWaves sit below one pixel per Airy FWHM (0.77 and 0.66 in the table above) and both are seeing-limited by an order of magnitude: 2.5″ at OHP against a 0.213″ Airy FWHM, 1.16″ at Palomar against 0.105″. So the spikes are now *modelled* rather than *absent*, which is the error that was worth removing, but they will be faint and the sampling will do most of the erasing. Expect no dramatic change on either amateur instrument. The telescope this fix actually shows is Hubble, which has no atmosphere in front of it and where the spikes are the dominant PSF structure away from the core.

If PlaneWave ever publishes a figure, both entries change and nothing else does.
...
| RC20 | 275.4 mas | 213.3 mas | 0.77 | modelled on an estimated width (§7.113.1), **below one pixel** |
| CDK1000 | 159.2 mas | 105.5 mas | 0.66 | modelled on an estimated width (§7.113.1), **below one pixel** |
...
**Truncation, and the bug it hid.** Spikes formally run across the whole frame while the kernel is bounded by `MaxKernelRadiusPx`, so the kernel carries them only within its own support and is renormalised as always: no flux is lost, but the far spike wings are not drawn. Same computational bound the Airy wings already have (§7.11).
...
4. **Detector**: shot noise, dark current, gain, read noise, cosmic rays, blooming, CTI, defects, and persistence (modelled, off on every instrument for want of a published amplitude — §7.5166).
...
- **Persistence / residual surface image**: modelled in full at §7.5166 (`Core/DetectorPersistence.cs`), and **off on every instrument** because no detector on this roster has a published amplitude — WFC3/UVIS having been tested and found not to show it (ISR WFC3 2005-10), the rest simply never measured. It is the only effect in this chain that depends on the observing *sequence* rather than on the exposure, and the only one holding state between frames.

  *The failure this replaces, kept because the lesson is the reason the new model is shaped as it is.* An early version ported Pyxel's default persistence trap model (time constants and species proportions in the spirit of **Fixsen, Offenberg, Hanisch et al. 2000, PASP 112, 1350**). That model is tuned for **HgCdTe near-infrared arrays**, a technology known for pronounced latency — not for a back-illuminated Sony CMOS whose main advantage is negligible image lag. Worse, the ported proportions summed to exactly 1.0, which is a Pyxel-internal weighting for decomposing an IR array's ghost signal among trap species and **not** a bound on the fraction of the current frame to subtract. Treated as one it produced literal runaway darkening: repeated exposures fed the slow (τ = 100–10000 s) species enough elapsed time to keep approaching their share of the signal on every capture, with no equilibrium below near-total signal loss. It was removed rather than re-tuned. The model that replaces it fixes each of those three faults by construction — it is CCD surface-trap physics rather than an IR array's, its capture is bounded by a finite trap density rather than by a normalised weight, and the charge it captures **leaves the well it came from**, so the sequence conserves charge instead of draining it.
...
| PRNU from a measured map | **equal** | was the one **worse** row in this table. Pyxel's `fixed_pattern_noise(filename=...)` loads a real per-pixel flat; `Core/FitsImageReader` + `Core/MeasuredFlatField` now do too, from FITS at any standard BITPIX, with the pedestal removed before normalising and the response of a saturated pixel treated as unmeasured rather than as high (§7.513) |
...
#### 7.513 Loading the observer's own flat (`Core/FitsImageReader.cs`, `Core/MeasuredFlatField.cs`)

**This closes the one row in §7.512's table that read "worse", and it is the only gap in the detector chain that closes without a citation** — because the source is not a paper, it is the user's file.

**Why a measured flat is a different kind of thing, not just a better model.** The parametric map can only produce the structure it was told about: a white photo-response spread and a cosine-fourth falloff. What a real flat contains is mostly neither. Concentric **tree rings** from radial dopant variations in the silicon ingot and the **brick-wall** imprint of laser annealing (§12 item 57); **dust motes**, the out-of-focus shadows of dust on a filter or window; **accessory vignetting** from undersized filters, a narrow drawtube or an off-axis guider (§12 item 61). None of those three is a property of the *instrument*. They are properties of one optical surface on one night, which is why no specification publishes them, why borrowing another device's would put a specific visible wrong structure into every frame, and why the observer's own file is the only source that has them.

**The reader is a real FITS reader**, not a fast path for one file this project happens to write. Primary HDU, `NAXIS=2`, every `BITPIX` the standard defines (8 unsigned, 16/32/64 signed two's complement, −32/−64 IEEE), all big-endian; `BZERO`/`BSCALE` applied as `physical = BZERO + BSCALE × stored`; `BLANK` mapped to `NaN`. A slash inside a quoted string is not a comment, and a doubled quote is one literal quote. Non-conforming `SIMPLE`, a cube, an undefined `BITPIX`, a truncated data segment and a zero `BSCALE` are each refused **by name**, never guessed at or zero-filled, because the person who has to act on the message is holding the file.

**`BZERO` is not optional decoration.** Unsigned 16-bit data is stored in FITS as signed shorts with `BZERO=32768`, which is what essentially every astronomy camera writes. A reader that ignores it reads a 60000-count flat as −5536.

**Three decisions in turning a frame into a response map**, each of which is a way to get a flat wrong:

* **The pedestal is removed first, and it is a required input.** A raw flat carries the readout's zero offset on top of the light, so the ratio of two pixels' raw counts is their response ratio *pulled toward unity* by the pedestal. Measured: at a 3000 ADU pedestal on a 20000 ADU flat, a pixel that really responds 10 % low reads **8.7 %** low. It is required rather than optional because nothing in the file says whether it has already been subtracted; it is taken from the file's own `PEDESTAL`/`BLKLEVEL` card when there is one, and the fallback to the instrument's modelled bias is logged rather than assumed silently.
* **Normalised by the mean, not the peak.** The chain wants "fraction of the array's mean response", which is what lets the flat divide out without changing a frame's level. Normalising by the maximum would rescale every calibrated frame by the reciprocal of the brightest pixel's response, which is a hot pixel as often as not.
* **A saturated pixel's response is unmeasured, not high.** The well clipped before the response could express itself. Such pixels are counted, reported, and held at unity — neutral changes nothing, zero would erase the pixel, extrapolation would invent it.

**A measured flat replaces the modelled one; it does not multiply it.** §7.51 already argues that the illumination falloff and the photo-response spread are stored as one map because their product is precisely and only what a flat frame measures. A real flat therefore already contains both, and applying the model on top would apply the modelled vignetting twice.

**A file that exists but cannot be used is loud, and does not fall back quietly.** Someone who put a file there meant to calibrate against it; substituting a parametric map silently would leave them reducing against a flat they believe is theirs and is not.

**The noise diagnostic, and the version of it that was wrong.** A single flat *sub* carries shot noise, and normalising it stamps that noise into every subsequent frame as fixed pattern. The floor is 1/√N in electrons. The first implementation compared the whole-array sigma against that floor and flagged a **clean** flat as noisy, because a clean flat also has a low sigma. What the diagnostic actually needs is the **high-frequency** scatter, estimated from differences between horizontally adjacent pixels and divided by √2: vignetting, tree rings and a dust mote's penumbra are smooth across one pixel and drop out, while uncorrelated shot noise contributes all of it. The test is `HighFrequencySigma ≥ 0.7 × floor`, resting on the fact that a single sub cannot scatter below its own shot-noise floor — one that does was combined from several, which is what a master flat is.

**Where a flat goes.** `GameData/ExoInstruments/PluginData/Flat_<camera>_<filter>_bin<N>.fits`. All three are in the name because a flat belongs to one optical train at one filter at one binning: the dust that makes a real flat worth having sits on a filter, and the map is per frame pixel rather than per sensor pixel. A flat of the wrong size is refused rather than resampled, since interpolation would blur exactly the fine structure that is the reason for loading one.

**Validated in `tools/flat-tests`** (27 checks) **and cross-validated against astropy 6.0.1.** The C# harness writes the FITS files and reads them back, which cannot catch a reader and writer sharing a misunderstanding of the format; `compare_astropy.py` reads the same twelve files with `astropy.io.fits` and every one agrees to **exactly zero**, including the `BLANK` pixel, which must come out undefined on both sides rather than as a number. Same argument as §7.5165's and the optics cross-validations: agreement with an independent implementation is evidence about the standard rather than about internal consistency.

### 7.515 Fringing — the sky's own spectrum, printed on the silicon (`Core/Fringing.cs`)

A thinned CCD is an etalon nobody meant to build: light that reaches the silicon without being absorbed reflects off the far surface and interferes with what is still arriving, modulating the response by `cos(2πP/λ)` with `P = 2nd` the optical path across the layer. In the blue silicon absorbs everything within a micron and there is nothing to reflect; past ~800 nm the absorption length approaches the layer thickness and the modulation becomes the dominant flat-field structure.

**Why it does not average away, and why it is the sky's effect as much as the detector's.** Over a broad passband the phase runs through many turns and a *smooth* source's fringes cancel. The night sky is not smooth: past 700 nm it is a picket fence of OH Meinel bands, and each line samples the modulation at one phase rather than averaging over it. This pipeline can compute that integral rather than assert it, because it already carries the airglow line spectrum at 0.1 nm sampling (§7.3, `AirglowTable`).

**Two ESO measurements, from two papers, and the check that they agree.** Walsh, Kuntschner, Jehin, Kaufer, O'Brien, Riquelme & Smette (2008, *2007 ESO Instrument Calibration Workshop*) fed a monochromator into FORS2's calibration unit: nothing visible at 774 nm, then 2.2 % at 876, 3.0 % at 906, 5.1 % at 926, 7.0 % at 956, 7.5 % at 986 nm, with a fringe **period of 2.9 nm** and the closest fringes ~40 px apart. Downing, Baade, Sinclaire, Deiries & Christen (2006, SPIE) state the device: *"The MIT/LL CCID-20 is a 40 µm thick high resistivity deep depletion CCD."* A period `Δλ` at `λ` implies `P = λ²/Δλ`, so 2.9 nm at 950 nm gives 311 µm of optical path and, through silicon's dispersion (n = 3.585, Green 2008), a **43.4 µm layer against the 40 µm published — 8.5 %**. A spectroscopic measurement and a fabrication figure that were never compared before, agreeing. The published thickness is the one carried.

**One integral turns one measurement into the other.** `F(P) = 1 + [∫ S R (A/2) cos(2πP/λ) dλ] / [∫ S R dλ]`, with S the sky, R the response and A Walsh's monochromatic curve. Run against real passbands and the real airglow, it reproduces all three of the manual's prose statements at once:

| passband | this model | ESO's FORS2 manual §2.7.2 |
|---|---|---|
| R_SPECIAL (655/165 nm) | 0.000 % | — (below the 774 nm onset) |
| I_BESS (768/138 nm) | 0.057 % | *"fringes are hardly visible"* |
| z_Gunn (910/13 nm) | **0.659 %** | *"the fringe amplitudes are below 1%"* |
| 1 nm slit at 956 nm | **5.97 %** | *"of the order of 5% in the worst cases"* (spectroscopy) |
| Luminance (unfiltered, 330–1100 nm) | 0.133 % | — |

**Neither sky washes out monotonically, and for two different reasons** — a result the harness produced by failing twice. Integrating `cos(2πP/λ)` over a top-hat gives a **sinc envelope in bandwidth**, so even a perfectly smooth source collapses wherever the band spans a whole number of fringe periods and revives between those zeros: at 956 nm the period is 3.19 nm and the smooth sky duly bottoms out at a 3 nm band (0.185 %) before rising to 0.461 % at 10 nm. The real sky adds a second, unrelated structure: its OH bands sample the cosine at particular phases, so widening the band can admit more coherent light than it cancels. **At a 3 nm band the two skies differ by a factor of 11.** Together these are why fringing has no safe bandwidth, why observatories measure it per filter rather than predicting it, and why a night's own airglow changes it.
...
Nothing here is sourced from a datasheet and nothing needs to be: a dither pattern is a choice an observer makes, not a property of an instrument. What *is* a property of the instrument, and is deliberately not modelled, is the mount's own pointing error (§12 item 72).
...
### 7.5166 Persistence — the first effect that needs the detector to remember (`Core/DetectorPersistence.cs`)

Not every electron a bright pixel collects leaves with the readout. Under some clocking conditions charge reaches the silicon-silicon-oxide interface, where the lattice terminates and leaves states in the band gap, and is held there. Those states empty **thermally**, on a timescale set by their depth rather than by the readout, so the charge returns to the well over the exposures that follow and is read out again as a ghost. This is the **residual surface image (RSI)**, and it is distinct from the residual **bulk** image (RBI) that makes infrared arrays and deep-depletion CCDs need a light flush between exposures. Only RSI is modelled, following the one modern quantitative characterisation of a scientific CCD's residual images (*Characterization of Residual Charge Images in LSST Camera e2v CCDs*, arXiv:2502.05418), which reports RSI and states RBI is not seen in that camera.

**Why it is architecturally different from everything else in §7.5.** Persistence is the only term in the chain that depends on **what was observed before**. A bias, a dark and a flat are properties of the detector and the exposure; a residual image is a property of the observing *sequence*. Two identical exposures of the same field differ if one followed a bright star and the other did not, so no master frame can carry the correction and the standard calibration set cannot touch it — every handbook that documents it documents it as a **scheduling constraint**, not as a correction. It is therefore the first effect in this pipeline to require inter-exposure state, which `SolarSystemCameraTexture` now carries as two per-pixel trap populations that survive between frames and are discarded only when the instrument or the binning changes.

**Each part of the form is the shape the measurements have, not a choice.**

| feature | why | source |
|---|---|---|
| capture is **thresholded**, not proportional | every published measurement reports residuals after *saturated* sources and nothing below; a proportional model would put a ghost under every source in the frame | WFPC2 IHB §4.5: a concern "only for stars that were saturated in a previous image" |
| capture **saturates** at a finite trap density | a pixel driven to 100× full well does not trap 100× the charge | WFPC2 IHB §4.5: no measurable residual half an hour after 100× full-well overexposures |
| release is a **sum of two exponentials** | two exponentials is what two trap depths look like | arXiv:2502.05418 fits the decay as a sum of two |
| the two are carried as **two separate populations** | see below | design consequence of the above |
| release runs in **elapsed time, not frames** | the states empty thermally, so the clock empties them, not the shutter | arXiv:2502.05418 states the timescale in seconds, with the frame count as a gloss: "well over a hundred seconds (around ten 15s exposures)" |

**Why two arrays rather than one number with a two-term law.** A single exponential composes with itself over any split of an interval; a *sum* of two does not. Storing one trapped charge and decaying it by `w·e^(−t/τ_f) + (1−w)·e^(−t/τ_s)` is therefore correct only at the cadence the fit was measured at, and has no defined state after a partial decay. The harness measures both halves of that claim: two populations reproduce a 300 s release from 3×100 s to **0.00×10⁰** relative, and the one-array two-term law misses the same test by **2.3×10⁻¹**.

**What is published for this roster, which is why the effect is off on every instrument.** The same situation as brighter-fatter (§7.5165), recorded the same way: the mechanism is modelled and waiting for a number.

| instrument | detector | state |
|---|---|---|
| HST / WFC3/UVIS | e2v CCD44-82 | **measured absent.** ISR WFC3 2005-10 obtained dark images following highly saturated PSF images specifically to evaluate image persistence in the CCDs, and found none significant, consistent with previous ambient testing. Carried as its own flag, not as a `NaN` |
| RedCat 51 / RC20 / CDK1000 | Sony IMX492 | unpublished. A pinned-photodiode CMOS empties its diode completely into the sense node by design, and a scientific CMOS measured in X-ray shows lag confined to the next frame and below 0.05 % — but neither Sony nor ZWO publish a figure for *this* device, and an architectural expectation is not a measurement |
| FORS2 | MIT/LL CCID-20 | unpublished. Checked against the FORS2 user manual |
| SPHERE/ZIMPOL | ZIMPOL CCDs | unpublished. Checked against Schmid et al. (2018) |

**A measured absence is not a `NaN`,** and conflating them would throw away a real result. `PersistenceMeasuredAbsent` is set on WFC3/UVIS alone and the harness asserts that exactly one instrument carries it and that it carries no amplitude beside it. Note that HST's much-cited persistence literature is **WFC3/IR's HgCdTe array**, the other channel of the same instrument: a CCD and an infrared array trap charge in different places for different reasons, and none of it transfers.

**Two real measurements deliberately not borrowed.** WFPC2's Loral CCDs give **0.3 % ± 0.1 %** of the original star flux, clearing within **1000 s** at −70 °C; LSSTCam's e2v CCD250 gives a trail residual **at the scale of 10 e⁻** above sky, taking over a hundred seconds to clear. Both are carried in `DetectorPersistence` as named reference points, used by the harness to check the model reproduces their clearing behaviour and used for nothing else. Defaulting to either would be borrowing another device's interface physics — the thing this codebase refuses to do for brighter-fatter, for PRNU and for fringing.

**Where it sits in the chain.** Release is applied **after the Poisson draw and not through the flat field**, for the two reasons that already separate dark current from scene light: these electrons never travelled through the optics, so no pixel's photo-response scales them, and they are not a fresh counting process, so drawing them from a Poisson mean would give them a variance they do not have. Capture is taken from the well **after blooming and before transfer**: blooming is what caps the charge a pixel actually held, transfer is what happens to it on the way out, and the traps see the charge that sat there.

**Validated in `tools/persistence-tests`** (25 checks): capture is zero below and at threshold, monotonic above it, saturates exactly at the trap density, and takes nothing when the traps are full; release never exceeds what was trapped and is exact under any split of the interval; the model clears to **0.92 %** at WFPC2's 1000 s and **0.037 %** at its half hour while still holding **27 %** at the CCD250's own 150 s scale; and the shipped catalogue has the effect off on all six instruments, for a recorded reason each.
...
So the declared form is **substantially too optimistic below about 3 λ/D of arc** — a factor of two at 1.4 elements, a factor of six at 0.4 — and **slightly pessimistic above it**, crossing over near 3. §12 item 65 is rewritten accordingly: the form is no longer merely declared, it is bounded by measurement, and the measurement is the one to use where a sequence exists to reduce.
...
The RA/Dec zero point remains the arbitrary convention of §1 and §12 item 1 on stock. The WCS is internally exact and self-consistent — a source at a given catalogue position lands where the header says — but on stock the frame is not tied to the real sky.
...
**This list is live and is pruned rather than annotated.** An entry that has been closed is *deleted*, not struck through and kept, so that the list is at every moment an exhaustive statement of what the pipeline does not model rather than a history of what it once did not. Where closing something left a smaller limitation behind, that remainder appears as its own entry. Where an entry's *justification* turned out to be wrong rather than its conclusion — brighter-fatter, item 23 — the entry is rewritten and says so. The history is in the git log and in the section each entry points at; it is deliberately not here.

Numbered items are referenced elsewhere as "§12 item N". The subsections **§12.1** and **§12.2** below are separate long-form discussions and are not list items.
...
2. **No weather simulation** in the generic (exoplanet-instrument) observing forecast — only the solar-system-photography forecast (any ground instrument, §7.4) factors in real EVE cloud cover, and even that assumes clouds persist unchanged into the future (no forecastable weather model exists to query).
...
9. **The exoplanet direct-imaging instrument's speckle field is uniform pseudo-noise**, and the solar-system astrograph's is not. `Visualization/DirectImagingTexture.cs` draws each pixel's speckle term as a uniform deviate on `[0,1)` scaled by twice the local contrast floor, so it has the wrong distribution, a non-zero mean and no photon noise on top. `Core/SpeckleField.cs`, built for SPHERE/ZIMPOL (§7.52, §7.522), draws the physically correct modified Rician with measured decorrelation timescales. **The mod therefore carries two speckle models of very different quality**, and the older one has not been ported to the newer. Closing it means routing `DirectImagingTexture` through `Core.SpeckleField`, which is now a wiring job rather than a research one.
10. **SPHERE/ZIMPOL's filters are still top-hats** of their published FWHM and peak, while FORS2's three broadband filters now carry ESO's measured curves. Not a modelling choice: the ZIMPOL curves were simply not located, and the top-hat is the correct treatment until they are (§7.02).
11. The **RC20 and CDK1000 have real spider vanes that are not modelled**, because PlaneWave publishes no vane width and spike brightness scales as the vane area squared: guessing the width would be guessing the effect. Both are set to zero vanes and declared, the same treatment the CDK1000's astigmatism already gets (§7.113).
12. The **VLT vane count of four is read from the telescope's structure, not quoted**: ESO's technical prose describes "metallic beams called spiders" without giving a number. The vane *width* is on firmer ground, being derived from published scaled pupil masks by a scaling that reproduces the ELT's independently published figure to 4 % (§7.113).
13. The vane **width** is a literature value with real spread: 50 cm per Schwartz et al. (2018), against 54 cm in METIS phase D simulations and 40 cm in at least one published pupil figure. Spike brightness scales as the vane area squared, so that spread is a factor 1.8 on an effect of order 1e-4 of peak (§7.112).
14. The profile's average over a pixel's azimuthal extent is exact within 6 px of the source and taken in its narrow-angle limit beyond, agreeing with a brute-force average over a real square pixel to **6.7×10⁻⁴ of peak intensity** across every plate scale the display produces (§7.111).
15. **One unanchored constant remains in the solar-system sensor chain**: astigmatism's pixel amplitude at the frame corner, used on the RC20 alone, where no published optical prescription specifies it to the needed precision (§7.1). Everything else in that chain is a real electron count against a real full well, read noise and dark current (§7.0, §7.5).
16. **CTI is a simplified single-trap-species model**; Pyxel's own real CDM (which this is adapted from) uses full SRH capture physics in real electron counts across multiple trap species.
17. **Cosmic ray incidence angle is isotropic-sampled**, not derived from a real particle angular-distribution model (matches the fact that Pyxel's own shipped angle model is an unimplemented stub) — though the *rate* is now a real derived quantity (sea-level muon flux over a cited real sensor's pixel area, see §7.5).
18. **Habitable-zone polynomial fits are only valid 2600K–7200K** — no extrapolation outside that range (returns null instead).
19. **BSC5-derived decoy stars carry no mass/radius/distance data** (pre-Hipparcos catalog, no such columns) — always treated as "unknown," never invented.
20. **Transit duration (T14) assumes a circular orbit** — no eccentricity term, despite eccentricity being tracked elsewhere in the same star's data.
21. **All positional/name catalog matching uses small-angle and string-heuristic approximations** — ambiguous matches are refused (flagged for review) rather than guessed, but the matching itself isn't a rigorous spherical-trigonometry/fuzzy-matching pipeline.
22. **Deterministic hash-based "randomness"** for stellar activity level, rotation period, spot phase, RM spin-orbit angle, and direct-imaging pointing/position-angle — reproducible per star, not drawn from any measured distribution beyond the *range* the draw is confined to.
23. **Brighter-fatter has no published amplitude for any detector on this roster.** The mechanism is modelled and validated against the one device it *is* published for (§7.5165): ESO measured 1.4 % horizontal, 2.2 % vertical and 10 % summed correlation at 90 ke⁻ on an e2v CCD44-82, and the model reproduces all three plus their stated 10 % gain over-estimate. But the same paper tested the MIT/LL CCID-20 that FORS2 uses and reports its autocorrelation nowhere, and nothing is published for the IMX492 or ZIMPOL's CCDs. So every instrument carries `NaN` and the effect is off. *(This replaces an earlier entry which claimed no generic published values existed at all, and that none of these instruments do stellar photometry; both were false.)*
24. **The solar-system astrograph's lucky-imaging sharpness scoring window (60% central region) and outlier-trim fraction (2%) are algorithmic engineering choices**, not measured quantities — the *operator itself* (variance of Laplacian) and the *keep fraction* (30%, literature range 1–60%) are literature-sourced, same distinction as other robust-statistics parameters already in the codebase (e.g. the sky-background trim fraction).
25. **The real photon-flux magnitude model (§7.0) treats a KSP `CelestialBody.albedo` as a rigorous geometric albedo** in the H-G planetary-magnitude sense, and its `.Radius` as a perfect sphere — the best available real input data from the game itself, but not a measured planetary albedo in the astronomical sense.
26. **The Lambertian phase law (Russell 1916) assumes a perfectly diffuse, uniform-albedo sphere** — real solar-system bodies (especially airless, cratered ones) show real deviations from this (opposition surge, limb darkening/brightening) that this mod doesn't model.
27. **SPHERE's adaptive-optics resolution (§7.013) is applied as a fixed real "good conditions" FWHM regardless of current airmass** — the cited papers report a range (25 mas headline, 22-28 mas across bands), and real AO correction does degrade somewhat at higher airmass, but no published relation exists to model that degradation, so the headline figure is used unconditionally rather than left unmodeled or invented.
28. **Charge-domain binning is assumed for the DISPLAY binning of every instrument**, in the binned-full-well scaling of §7.05: the real capacity gain from combining N×N physical pixels depends on where in the readout chain the summation happens, and this is modelled as charge-domain throughout. *The sensor's own internal binning is no longer assumed*: `SensorNativePixelsPerSide` records that the ASI294MM Pro's 4144×2822 format is the IMX492 summed 2×2 in the sensor, proved by its full well being four times the single pixel's, and that distinction is what makes its PRNU and DSNU figures land on the right pixel (§7.51). What remains unmodelled is the display binning on top of it.
29. **Telescope-switch exposure rescaling (§7.011) assumes the player wants equivalent total collected light**, the same assumption a real exposure-time calculator makes — it does not know the imaged body's actual real-time brightness, so a target that's genuinely much brighter/fainter than whatever the previous instrument was pointed at can still land outside a comfortable exposure after rescaling.
30. **VLT SPHERE's Blue filter slot is omitted rather than approximated** (§7.012) — ZIMPOL genuinely has no real broadband blue filter, so rather than reuse a nearby real filter's numbers under a misleading label, that filter position simply isn't offered for this instrument.
31. **Solar-system body marker decluttering (§8.1) is a chart-display convenience, not real astrometry** — the small on-screen separation it introduces between an overlapping planet and its moons has no physical meaning and is never used by anything outside chart rendering and click hit-testing.
32. **No rendered star field ships at all** — the catalogue is user-built from Gaia DR3 (§7.016), because the useful depths cannot be distributed (202 MB at G<13, 1.9 GB at G<16). A player who installs nothing photographs an empty sky. Giving them a plausible field at zero download needs a statistical population from a Galactic star-count model — a real 30 s RC20 sub reaches several magnitudes fainter, so a frame holds about five real stars where a real one would hold hundreds. Closing this needs a Galactic star-count model generating a statistical faint population (Bahcall & Soneira 1980; Besançon; TRILEGAL), not a larger catalogue (§7.14).
33. **Cloud veiling is a gain on the existing sky (×3 at full coverage), not an absolute cloud brightness** — the pipeline has no ground-light model to derive one from, and scaling the term whose light the cloud is actually reflecting is preferable to inventing an absolute figure (§7.3).
34. **The twilight brightness gradient (0.6 mag per degree of solar altitude) is a straight-line approximation** to Patat et al. (2006)'s measured curve over the -18° to -12° span, not a fit to it (§7.3).
35. **Optical throughput is modelled only where a figure is published**, and the unmodelled factors are enumerated per instrument in §7.001's table rather than lumped into one fudge: the RedCat 51's whole refractive glass path, every instrument's relay/corrector/Barlow train, FORS2's collimator+camera relay, and every broadband filter's peak transmission except FORS2's Hα. Each of those sits at 1.0, which means *not modelled*, not lossless — so the limiting magnitudes remain optimistic, by less than before but not by zero.
36. **Mirror reflectivity is a single grey band-averaged figure (0.87)** applied to every instrument alike, because the source (Ma & Cai; Magrath 1997) quotes it as a band average over the full optical range. Real coatings have a wavelength-dependent curve and a real observatory's mirrors sit at a definite point in their own re-coating cycle; neither is resolved here. PlaneWave's optional enhanced coatings are not modelled at all, no measured curve being published for them.
37. **No published QE curve exists for the ZWO ASI294MM Pro**, so the RedCat 51 / RC20 / CDK1000 use its 90% peak flat across every band, which overstates each filter away from the peak. A measured curve for a sibling Sony back-illuminated CMOS exists (Alarcón et al. 2023) but normalising another chip's shape to this one's peak would be an assumption presented as a measurement (§7.001).
38. **Filter passbands are top-hats of the published FWHM at the published central wavelength**, scaled by the published peak transmission where one exists. The equivalent width is therefore exactly the published figure, and no unmeasured profile shape is imposed — but a real interference filter's finite edge slope and out-of-band leakage are not modelled (§7.0).
...
40. **A photographed body's reflectance is grey.** Its spectrum is integrated as the Sun's (5772 K, IAU 2015 Resolution B3), which is real physics for reflected sunlight, but a KSP `CelestialBody` carries one albedo with no wavelength dependence to read, so the reddening or bluing a real planetary surface imposes on reflected light is absent (§7.0).
41. **Airglow is integrated with a flat spectrum** because it is line emission (OI 557.7nm, OH Meinel bands) and no continuum shape could stand for it; the other three sky terms are integrated as scattered sunlight, which they are (§7.3).
42. **The exported WCS is exact and self-consistent but tied to the arbitrary RA zero point** of §12 item 1 on stock: a source at a given catalogue position lands where the header says, and the frame plate-solves against itself, but the pointing is not the real sky's unless the installed pack models the real solar system (§7.7.1).
43. **A stacked composite carries no WCS** (§7.7.1), because centroid registration on a moving body aligns the target and not the field, so no single pointing describes the stack. The individual subs it was built from each carry their own.
...
61. **Dust motes and accessory vignetting are absent from the MODELLED flat**, and are the clearest case of why loading a measured one (§7.513) is not merely a better model but a different kind of answer. Out-of-focus shadows of dust on a filter or window are the most recognisable feature of a real amateur flat, and undersized filters, a narrow drawtube or an off-axis guider are what produce the deep corners in most of them. None is part of any instrument's published specification, and their number, size and position are a property of **one night's optical surfaces** rather than of the instrument, so no specification could ever carry them and no model can legitimately invent them. The consequence for the default map is stated plainly: **the modelled flats for the three amateur tubes are far flatter than the ones their real owners take.** What has changed is that their real owners can now supply the real one (§7.51, §7.513).
62. **The FORS2 shutter's illumination non-uniformity is not modelled.** The manual states the rotating half-segment shutter "guarantees uniform illumination of the CCD to the 1 % level or better for exposure times as short as 1 sec", which bounds the effect and makes it exposure-dependent — a real term that grows as the exposure shortens toward the 0.25 s minimum. The two-dimensional shutter map is not published, so only the bound is known and the shape would have to be invented (§7.51).
63. **The converter has no differential or integral non-linearity.** Quantisation and clipping are modelled exactly (and agree with Pyxel's converter on every level tested), but a real ADC's code widths are not perfectly uniform. No DNL/INL figure is published for any converter on this roster (§7.51, §7.512).
64. **The ASI294MM Pro's HCG mode is not offered.** The real camera switches its sense node at ZWO gain 120, reaching 1.2 e⁻ of read noise with the well falling to roughly 19,700 e⁻ and `K` to about 1.2 e⁻/ADU. This pipeline models the low-gain configuration throughout, with the gain slider acting as an analogue gain ahead of a fixed converter; the second conversion-gain configuration is a discrete hardware mode, not a point on that curve, so it cannot be reached by interpolation and is absent rather than approximated (§7.5).
65. **The ADI self-subtraction throughput has an analytic form that is now bounded by measurement.** `n/(n+1)` in the arc length is still what `SelfSubtractionThroughput` returns, and it is still a declared shape rather than a fit; what has changed is that §7.5205 now *measures* the throughput by injection and recovery, and the two disagree in a known direction. The form is **substantially too optimistic below ~3 λ/D of arc** (0.588 against a measured 0.290 at 1.43 elements; 0.263 against 0.051 at 0.36) and slightly pessimistic above it. It also has no frame-count term, while the measurement shows one (0.514 at 3 frames falling to 0.271 at 31, at fixed rotation). Use the measurement where a sequence exists to reduce; the form is for reasoning about magnitudes.
66. **The 4QPM phase masks are declared and not modelled.** ESO publish their design wavelengths (666 and 823 nm) but no attenuation curve, and a four-quadrant phase mask's attenuation away from its design wavelength is the whole of its behaviour (§7.52).
67. **The coronagraphic mask's dust and suspension wires are not rendered.** Both are documented facts about the real masks — dust on the deposited small masks, 34 mas wires on the suspended large ones — and neither is renderable: the dust pattern is a property of one particular October 2014, and the wires' position angle is not published (§7.52).
68. **Wind speed is a constant, 4 m/s.** The atmospheric speckle lifetime is 0.6 D/v and the pipeline has no wind model to read v from, so it uses the speed Milli et al. (2016) report for the very sequence their decorrelation timescales were measured under. The model therefore runs at the conditions its own numbers were taken at rather than at an invented default, but it does not vary with the weather (§7.522).
69. **The speckle modulation is applied to the whole signal plane, including a resolved target's disc.** A real extended source averages over the speckles its own light produces, suppressing its granularity by roughly the number of resolution elements it covers; that suppression is not modelled. Correct for the point sources a coronagraph is pointed at, an overestimate of the granularity on a resolved body (§7.522).
70. **The fringe map's realisation is drawn, not measured.** Its amplitude and its spatial scale are both Walsh et al.'s measurements; the particular pattern of thickness variation across one piece of silicon is not published for any detector and is drawn from the sensor's serial seed, exactly as the photo-response and defect maps are (§7.515).
71. **Fringing is modelled on FORS2 alone.** It needs a published silicon thickness, and neither Sony (for the IMX492) nor Schmid et al. (for ZIMPOL's CCDs) give one (§7.515).
72. **Mount pointing error is not modelled.** Periodic error from a worm gear, guiding residual and flexure are all real and all instrument-specific, and no mount model is published for any tube on this roster - the catalogue names optics and cameras, not mounts. Dithering (§7.516) is modelled because it is an observer's choice needing no instrument parameter; the errors it partly mitigates are not (§7.516).
73. **The photometric zero point carries no colour term.** A filter's effective wavelength depends on the spectrum behind it, so a real zero point is a function of colour; fitting one needs standard stars of known colour and a second passband, and what is fitted here is an uncertainty-weighted mean offset (§7.517).
74. **Source detection is the simplest thing that works.** Local maxima above a sigma-clipped threshold, with a one-resolution-element exclusion. No deblending, no PSF fitting, no moving-object rejection. Detection is not what §7.517 exists to validate, and anything cleverer would be a second algorithm needing its own validation (§7.517).
75. **The uncertainty does not model centroid jitter.** At low signal-to-noise the refitted centroid wanders between realisations, so the aperture moves and captures a varying fraction of the flux. The harness measures the resulting excess at +3.5 % in sigma at 3000 electrons, falling to nothing by 10^5 (§7.517).
...
77. **Source matching is nearest-neighbour from a supplied guess, not a blind solve.** It cannot place a frame whose pointing is unknown, which is the problem astrometry.net exists for; this pipeline always has a commanded pointing, so the problem solved is refinement (§7.518).
78. **The ADI reduction is median subtraction alone.** No PCA/KLIP, no LOCI, no LLSG, no reference-star differential imaging. Median-subtraction ADI is the technique the throughput model describes and the one Marois et al. (2006) introduced; the modern algorithms recover more and would each need their own validation (§7.5205).
79. **Brighter-fatter is first order and four-neighbour.** The expansion is in a·Q, about 0.8 % at the published coefficient and a 100 ke⁻ well, so second-order terms are ~6×10⁻⁵; diagonal neighbours and longer-range correlations, which the published summed 10 % implies exist beyond the 1.4 %+2.2 % nearest pair, are not modelled (§7.5165).
80. **The zodiacal dust cloud measured is the Solar System's**, and the zodiacal light's slight reddening relative to the Sun (Leinert et al. 1998 Sect. 8.4) is not modelled. The positional dependence itself is closed (§13.3); these two are what remained of it.
81. **The coronagraph's peak attenuation is a consequence of the halo model, not a value enforced against ESO's measurement.** `ApplyCoronagraphMask` blocks the core and the Lyot-stopped pupil sets the PSF; what an observer would measure as R_coro then falls out of whatever the halo is at the mask rim. `tools/coronagraph-tests` checks the *table* of ESO's measured 110–3000 but never checks that a rendered frame reproduces it, so the emergent value is unverified (§7.52).
82. **ADI derotation is bilinear and smooths the field it turns.** `tools/coronagraph-tests` §D1 measures a 1.3 % relative residual over a rotate-and-rotate-back at 7.5° and 30°, and zero at 0° and 90° where no interpolation happens. A derotated frame is therefore marginally quieter than the one it came from, which flatters a contrast curve by an amount that has not been separated out (§7.5205).
83. **The ADI reduction aligns to the sequence's own first frame, not to north.** A caller who wants north up must rotate once more afterwards by that first parallactic angle. Nothing in the pipeline does, so a reduced frame carries no meaningful WCS (§7.5205).
...
- **`TryBuildSiteBasis`** still uses the ground site, always. That basis is not the observer's
  horizon; it is how this mod's fictional RA/Dec frame is *defined* (§12 item 1), and
...
### 13.75 The second channel — WFC3/IR, and the first detector here that is not a CCD (`VisualTelescopeCatalog.HubbleWfc3Ir`, `Core/InfraredArray.cs`, `Core/HgCdTePersistence.cs`)

**This is not a second telescope, and it is not a second instrument.** WFC3 has a **Channel Select Mechanism**: the beam goes to the UVIS CCDs or to the IR array, never to both at once. So the 2.4 m primary, the 0.330 obstruction, the four spider vanes and their width, the three primary-mirror pads, the orbit and every avoidance angle and jitter figure are the *same numbers* as §13.7's, and the part config's `instrumentName` is what says which detector is at the focus. No new part, no new mesh, no new optics.

Everything from the detector inwards is different, and it differs by **which effects exist**, not by which numbers are plugged into one chain.

#### What an HgCdTe array does not have

A CCD clocks charge to a shared output. An infrared array reads every pixel where it sits, through the pixel's own amplifier. Three consequences, and the WFC3 handbook states each one outright:

| CCD effect | on WFC3/IR | handbook |
|---|---|---|
| charge-transfer inefficiency | **absent** | "IR detectors show minimal long-term on-orbit CTE degradation" |
| blooming along the column | **absent** | "no charge bleeding at saturation" |
| one destructive read | **absent** | sampled non-destructively up the ramp (MULTIACCUM) |

`RunDetectorChain` therefore forks on `VisualTelescopeSpec.Technology`. A full photodiode simply stops collecting, so saturation is a ceiling with nothing redistributed; there is no transfer to be inefficient at. Running an infrared array through the CCD branch would add two effects the device does not have.

#### What it has instead, all four measured

**Persistence, and it is the first instrument on this roster where the effect is ON.** Every other detector here carries `NaN` and waits for a number (§7.5166, §12 item 84). This one is measured, fitted and published *with its own error budget*. Long, Baggett & MacKenty (2015, WFC3 ISR 2015-15) give the "Fermi model":

`P(x,t) = A · [1/(e^(−(x−x₀)/δx) + 1)] · (x/x₀)^α · (t/1000 s)^(−γ)`

with `P` the persistence **rate** in e⁻/s at a time `t` after an exposure that left the pixel with fluence `x`. All five parameters are functions of the **stimulus exposure time**, which is the whole point of the 2015 report: the previous model depended only on fluence and elapsed time, which amounts to assuming traps fill instantaneously, and the measurements show they do not. Table 2 gives the fit at eight exposure times from 49 s to 1402 s, transcribed whole into `HgCdTePersistence`, and the report's own prescription between them is linear interpolation on exposure time — the same thing version 3.0.1 of STScI's persistence pipeline does.

**A different law from the CCD model, for a physical reason.** A CCD's residual surface image is charge at the silicon-oxide interface, released with the exponentials a trap depth implies (§7.5166 carries two populations). An infrared array traps charge in defects within the detector **diodes**, whose trap and release times form a broad continuous distribution, and the sum of that distribution is measured to decay as a **power law**. Two technologies, two measurements, two laws; one code path for both would mean choosing which measurement to contradict.

**The rate is integrated over the exposure in closed form, not sampled at its midpoint.** γ sits near 1 across the whole table, so over a long exposure taken soon after a bright one the rate changes by a large factor from start to finish. The harness measures midpoint sampling as **14.9 %** wrong on the case that matters, against the closed form agreeing with a 200 000-step Simpson quadrature to **8.3×10⁻¹⁶**.

**Interpixel capacitance**, from Hilbert & McCullough (2011, WFC3 ISR 2011-10) Table 2, measured on orbit from hot pixels in the SPARS200 dark reference file:
...
IPC is a **capacitive** coupling between sense nodes: a signal in one pixel raises the *apparent* signal in its neighbours with no charge moving. It is therefore neither diffusion nor brighter-fatter, both of which move real electrons, and it is applied at readout as a fixed linear convolution. Three things are kept that a tidier treatment would lose. The **anisotropy is measured** (identical above and below, identical left and right, the two pairs differing at four sigma) and is not averaged away. The kernel **sums to 0.9985 and is not renormalised** — that sum is the report's own, and forcing it to unity would silently change five published numbers to assert a conservation the measurement does not quite show. And it is a **convolution, not a correlation**: the report defines each cell as what a source at the centre contributes to the pixel at that offset, so the source is read at minus the offset. Writing it the other way flips the kernel, which on a kernel this nearly symmetric is the 0.0001 between the left and right couplings; `tools/infrared-tests` caught exactly that by asserting the point-source response cell by cell. Independently corroborated: Seshadri et al. (2008) measured a very similar HgCdTe device by resetting individual pixels and found 1.4–1.55 % adjacent and 0.13 % corner, against this report's 1.27–1.64 % and 0.11 %.

**Count-rate non-linearity** (reciprocity failure): **0.75 % ± 0.06 % per dex, with no apparent wavelength dependence, measured across 16 astronomical magnitudes** (Riess, Narayan & Calamida 2019, WFC3 ISR 2019-01, combining WFC3-IR/UVIS cluster photometry, white-dwarf synthetic magnitudes, ground-to-orbit Cepheid comparisons, all previous measurements and the WFC3 grism). Applied to the mean collected rate **ahead of the Poisson draw**, because it is a change in the detector's sensitivity rather than a loss applied afterwards: the pixel really does convert fewer of the photons it was sent, and carries the shot noise of what it actually collected.

**One unpinned constant, declared rather than hidden.** The *slope* is measured to sub-percent accuracy; the flux level it is measured *relative to* is wherever the photometric zero point was established, and ISR 2019-01 states the convention without a number — "flux zeropoints are established from standard stars which are about ten astronomical magnitudes (4 dex) brighter than faint, sky-dominated targets". 100 e⁻/s is used, a bright-standard-star rate on this detector sitting 4 dex above a sky-dominated source's ~0.01 e⁻/s, reproducing exactly the span that sentence describes. Nothing about the effect's shape depends on it; it sets where the correction passes through zero. §12 item 86.

**Read noise is not one number.** IHB 5.7 gives a SPARS200 ramp at ~20.0 e⁻ with 2 reads plus the zeroth and ~12.0 e⁻ with 15, and CDS alone at 20.2–21.4 e⁻. Fitting the ramp to more non-destructive samples averages the noise down, so it is interpolated between the two published anchors **in 1/√N**, the shape averaging N samples actually has, rather than linearly in N — which would be a curve of no physical origin that happens to pass through both points.

#### The detector, and three numbers that needed a decision

| quantity | value | source and reasoning |
|---|---|---|
| format | **1014 × 1014**, 18 µm | IHB 7.3. Not 1024: the outer 5-pixel rim is **reference pixels**, light-insensitive cells read out with the array to track bias and thermal drift. They are part of the detector and not part of the image. The downlink volume uses the true 1024², because they *are* read out |
| plate scale | 0.1278″/px | IHB 7.4 measures **0.135 × 0.121″**, anisotropic because the IR focal plane is tilted with respect to the beam. This pipeline carries one scale, so the **geometric mean** is used: it preserves the solid angle per pixel exactly, and solid angle sets the sky background, which dominates every deep near-infrared exposure. (§13.7 chose one *axis* for UVIS instead, because there the two axes belong to two different chips and the aperture most programmes use was the more meaningful pick. Here both axes are one chip.) The implied 129.6″ field lands between the handbook's own 123″ and 136″ sides |
| gain | **2.2515** e⁻/DN | DHB 7.2 says "only a gain setting of 2.5 e⁻/DN is supported", and then tabulates the *measured* per-quadrant effective gains, averaged over all epochs and monitored twice yearly: 2.28, 2.221, 2.24, 2.265. The frame is read through all four, so no single quadrant describes it, and their mean is what actually converts a DN back to electrons. Same reasoning as UVIS's four amplifiers, with one difference worth stating: there the measured values sat within 2 % of the handbook's summary figure, here they sit **10 % below the commanded setting**, so the choice changes the answer |
| full well | 78 000 e⁻ | IHB 5.7's saturation limit, the figure tied to the handbook's own 5 % departure-from-linearity criterion. Two other figures exist for other purposes and neither is wrong: DHB 7.2 gives ~80 000 e⁻, and ISR 2015-15 uses 70 000 as "the nominal saturation level" when counting saturated pixels — which is why `HgCdTePersistence` carries that one separately, against which its own parameters were measured |
| dark current | 0.048 e⁻/s | IHB 5.7: mode 0.045, median 0.048, mean 0.048. ISR 2015-15 quotes 0.046 (Hilbert & Petro 2012) and notes it varies 20–30 % for reasons not understood — a variation larger than the gap between the two published means, so no attempt is made to split them |
| temperature | 145 K | six-stage thermoelectric cooler. Far colder than any CCD here, which is what a 1.7 µm cutoff costs |

#### Throughput, and why the QE field reads 1.0

IHB 7.5 is explicit: "The throughput calculations include the *HST* OTA, WFC3 IR-channel internal throughput, filter transmittance, and the QE of the IR detector." That whole product is what Table 7.2's peak-throughput column gives, **per filter**, so it is carried in the per-filter peak transmission and the QE field is 1.0 with the mirrors uncounted. Putting a QE or a reflectivity there as well would square a loss that was measured once. Per-filter rather than one scalar preserves the real 0.52–0.56 variation across the set.

#### Filters, and the slot that is deliberately empty

Table 7.2's four wide filters onto the four broadband slots, in wavelength order: **F105W** (1055.2 nm, 265 nm wide, 0.52), **F110W** (1153.4, 443, 0.56), **F125W** (1248.6, 284.5, 0.56), **F160W** (1536.9, 268.3, 0.56). As with UVIS, the slot is a position on a wheel and the pivot wavelength carries the truth.

**There is no H-alpha slot**, and that is a physical statement rather than an omission. H-alpha is at 656 nm; this channel's filter set starts above 900 nm. The line is not merely unavailable, it is outside the channel entirely, and offering a slot labelled H-alpha with a 1.28 µm Paschen-β filter in it would be the mislabelling §12 already refuses for SPHERE's absent blue.

#### The colour path, and a real error this closed

Every band on this instrument lies beyond **830 nm**, the red end of the CIE 1931 observer's support. A least-squares fit of such bands to tristimulus values is a fit to a matrix of zeros: it returns something, and that something means nothing. `ColourComposite.CalibrationFor` now refuses to fit a colorimetric transform when any of the three colour bands falls outside the observer's 360–830 nm range, and reports why; composites fall through to the labelled-palette path, which makes no colorimetric claim. **That is what a real WFC3/IR colour image is.** The test is on the wavelengths rather than on the instrument, so any future infrared or ultraviolet band is caught by the same line.

#### Validation (`tools/infrared-tests/`)
...
- Bessell, M. S. (1990). *PASP* 102, 1181. Johnson-Cousins passband definitions — cited for what is *not* done: the V zero point remains a monochromatic normalisation rather than an integral over this curve (§12 item 39).
...
- *Characterization of Residual Charge Images in LSST Camera e2v CCDs* (2025), arXiv:2502.05418. Residual **surface** image on an e2v CCD250: two-exponential decay, residual at the scale of 10 e⁻ above sky in the trail region, clearing in well over a hundred seconds; states RBI is not seen in that camera. The source of the model's decay form in §7.5166, and a reference point it is validated against — not a default for any instrument here.
- STScI, *WFPC2 Instrument Handbook*, Sect. 4.5 ("Residual Image"). Residual flux 0.3 % ± 0.1 % of the original star flux for previously **saturated** stars, trapped charge escaping so residual images disappear within 1000 s at −70 °C, and no measurable residual half an hour after overexposures of 100× full well. The source of the thresholded, density-bounded shape of persistence capture (§7.5166).
- STScI, *Instrument Science Report WFC3 2005-10*, "WFC3 UVIS PSF Evaluation". Dark images following highly saturated PSF images, obtained to evaluate image persistence in the UVIS CCDs; **no significant image persistence** found, consistent with previous ambient testing. The one measured absence on this roster (§7.5166, §12 item 84).
- Long, K. S., Baggett, S. M. & MacKenty, J. W. (2015). *Instrument Science Report WFC3 2015-15*, "Persistence in the WFC3 IR Detector: An Improved Model Incorporating the Effects of Exposure Time". The Fermi persistence model, Eq. 1, with Table 2's five fitted parameters at eight stimulus exposure times from 49 s to 1402 s, and Table 3's per-interval fitting errors (0.0014 to 0.0275 e⁻/s). The whole of §13.75's persistence model, transcribed rather than approximated.
- Hilbert, B. & McCullough, P. (2011). *Instrument Science Report WFC3 2011-10*, "Interpixel Capacitance in the IR Channel: Measurements Made On Orbit". Table 2's 3×3 IPC kernel, measured on orbit from hot pixels in the SPARS200 dark reference file, with per-cell uncertainties and the published 0.9985 sum. Corroborated there against Seshadri et al. (2008) on a similar HgCdTe device.
- Riess, A. G., Narayan, G. & Calamida, A. (2019). *Instrument Science Report WFC3 2019-01*, "Calibration of the WFC3-IR Count-rate Non-linearity, Sub-percent Accuracy for a Factor of a Million in Flux". CRNL of 0.75 % ± 0.06 % per dex with no apparent wavelength dependence, measured across 16 astronomical magnitudes.
- STScI, *WFC3 Instrument Handbook* chapters 5 and 7, and *WFC3 Data Handbook* chapters 1 and 7. The IR channel's detector format and reference-pixel rim, plate scale and field, per-quadrant gain, full well, dark current, ramp read noise, Table 7.2's filters, Table 7.5's delivered PSF, the absence of charge bleeding and of CTE degradation, and the statement that the tabulated throughput includes the OTA, the instrument optics, the filter and the detector QE (§13.75).
