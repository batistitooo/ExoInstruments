...
- **Coma** (added this session): classical third-order (Seidel) off-axis aberration — transverse blur `∝ (field radius)²`, directed radially outward from frame center, zero at the centered target, worst at the corners for background stars. Chosen over GalSim's `SiliconSensor` electrostatic-vertex model because that model's calibration tables are sensor-specific proprietary data with no generic published values to port.
...
### 7.3 Sky background (multi-component, updated this session)

```
skyBackground = (twilightRamp·0.30 + moonSkyExcess·0.02 + airglow(0.004) + zodiacal(0.006)) · exposure · filterThroughput
```
- **Twilight**: ramps from the -12° capture cutoff down to -18° astronomical twilight.
- **Moon scattering**: now uses the *real* Krisciunas & Schaefer (1991) kernel from §5.3 (`MoonlightPollution.ScatteringKernel`), weighted by the true angular separation between the imaged body and each moon (previously a simplified altitude/phase-only proxy — this was corrected to reuse the same real formula the exoplanet instruments use).
- **Airglow**: fixed always-present baseline.
- **Zodiacal light** (new): fixed baseline representative of a typical high-ecliptic-latitude value per **Leinert et al. (1998)** tables — the mod has no real ecliptic geometry for Kerbol, so this is a constant rather than a position-dependent term.
...
- **Hot/dead pixels**: fixed defect map, seeded once from a constant (`20260721`), same defects every session — 1-in-3000 hot, 1-in-6000 dead.
- **Full-well blooming** (new): Pyxel's own model only hard-clips (`min(pixel, fwc)`, no redistribution — verified against their source, `pyxel/models/charge_collection/full_well.py`); since that gives nothing to port, this implements the standard real CCD behavior instead — charge above `1.0` spills into vertical (column-direction) neighbors, cascading up to 4 iterations, producing the classic bloom trail through a saturated star/planet limb.
- **Charge-transfer smear / CTI** (new): simplified single-trap-species version of the `nc`/`nr` capture/release structure from **Short et al. (2010)**'s CDM model (Pyxel's `pyxel/models/charge_transfer/cdm.py`). The real model needs SRH physics in actual electron counts (trap density, thermal velocity, electron-cloud volume) that don't exist in this [0,1] pipeline, so capture (2%/row) and release (35% of trapped/row) are constant fractions instead of the full physics, walked top-to-bottom per column.
- **Cosmic ray hits** (new): flat Poisson process (`0.15/s` expected rate — visually tuned, no real per-cm² flux since the 480×480 abstract sensor has no defined pixel pitch), isotropic incidence angle, random 2–14px track length, deposits a bright streak. Pyxel's own CosmiX/TARS angle model is an unimplemented stub in their shipped source, so isotropic sampling here is no less physical than upstream.
- **Persistence/ghosting** (new): real default trap time constants and species proportions from Pyxel's persistence model (`τ = {1,10,100,1000,10000}s`, proportions `{0.307,0.175,0.188,0.136,0.194}`). Release uses a **true exponential** `Q(t)=Q₀·exp(-Δt/τ)` rather than Pyxel's own small-step linear approximation (`diff = Δt/τ · empty_traps`), which would diverge over the multi-minute real-time gaps between manual RC20 shots that Pyxel's model was never designed for.
...
- **Lucky imaging** (new): each filter's subs ranked by peak-pixel value (**Fried 1978** selection metric — a sharper, less-blurred frame concentrates the same flux into fewer pixels, so peaks higher) and only the sharpest 30% kept before alignment/averaging (30% sits mid-range of the 1–60% literature practice, e.g. Baldwin et al. 2001). Always forces alignment on when active.
...
10. **RC20 sensor noise chain operates in abstract [0,1] units**, not real photon/electron counts — every effect (blooming, CTI, persistence, cosmic rays) had to be re-derived as a constant-fraction/analog approximation of the real physics (which needs real electron counts, trap densities, thermal velocities) rather than a literal port.
11. **Persistence and CTI are simplified single-trap-species models**; Pyxel's own real models (which these are adapted from) use up to 5 trap species with full SRH capture physics.
12. **Cosmic ray incidence angle is isotropic-sampled**, not derived from a real particle flux/angular distribution model (matches the fact that Pyxel's own shipped angle model is an unimplemented stub).
13. **Zodiacal light is a fixed baseline constant**, not position/season-dependent (no real ecliptic geometry exists for Kerbol in this mod).
...
- Leinert, C. et al. (1998). Zodiacal light reference tables.
