...
- **PSF**: the **exact annular-pupil Airy pattern** of the real ELT pupil, sampled through `Core/RadialPsfProfile` — the same closed form and the same `OpticalPsf.AiryIntensity` the rest of the roster convolves as a kernel (§7.11). Core radius, ring radii and ring amplitudes are all consequences of D, the obstruction and λ; none is a free parameter. See §7.111 for the pupil constant and the pixel-averaging that makes the pattern samplable on a raster whose plate scale varies by two orders of magnitude between targets.
- **Spider spikes**: 6-vane (ELT pupil) diffraction spikes, 60° apart, azimuthal Gaussian (σ=1.3°) × 1/r² radial falloff, amplitude `4e-4` relative to peak at 1λ/D. **The vane count is real, the amplitude is not**: it is a display constant. Deriving it means the two-dimensional Fourier transform of the pupil including its 0.5 m vanes, which is different machinery from the radially symmetric closed form the rings come from. Listed in §12.
...
**Known residual inconsistency.** `DirectImagingSimulator.DiffractionLimitArcsec` still returns `1.22·λ/D`, the unobstructed Rayleigh criterion, while the pattern's real first null is at `1.124·λ/D`. That constant gates `Resolvable` and scales every speckle floor and SNR in §4.1, so changing it would move detection outcomes in existing saves; it is left alone deliberately and recorded here rather than silently reconciled. Listed in §12.
...
**Catalogue: Tycho-2, and why not the BSC.** The Bright Star Catalogue stays exactly as it is and the detection pipeline is untouched — it is deliberately small so that hunting a transit remains a tractable game. It is simply the wrong catalogue for *rendering*: 9110 stars over 41253 deg² is 0.22 stars/deg², so an RC20 frame (0.068 deg²) contains 0.015 of them — one frame in 65 shows a single star. **Tycho-2** (**Høg et al. 2000, A&A 355, L27**) carries 2,557,476 stars including Supplement 1, 99% complete to V=11.0, at 61.9 stars/deg² measured — 4.2 per RC20 frame. Photometry is converted to Johnson at pack time by the transformation the Tycho-2 ReadMe itself prescribes (**ESA SP-1200 Vol 1 §1.3**): `V = VT - 0.090(BT-VT)`, `B-V = 0.850(BT-VT)`.

**Field of view, not catalogue depth, is what decides whether a photograph has stars in it.** Tycho-2's 61.9 stars/deg² is a fixed density; how many land on the sensor is then pure geometry, and the long-focus instruments lose that geometry badly:
...
9. **The direct-imaging frame's speckle and background are uniform pseudo-noise**, not physically-derived photon statistics. *(The PSF half of this entry is resolved: the frame now samples the exact annular-pupil Airy pattern of the real ELT pupil through the same `OpticalPsf.AiryIntensity` every other instrument convolves — §7.111. What remains is the noise.)*
   - **9a.** The six spider spikes drawn over it have a real vane count but an **assumed amplitude** (`4e-4` at 1λ/D, azimuthal σ=1.3°). Deriving it needs the two-dimensional Fourier transform of the pupil including its 0.5 m vanes, which is different machinery from the radially symmetric closed form the rings come from (§4.2).
   - **9b.** `DirectImagingSimulator.DiffractionLimitArcsec` still returns the **unobstructed** Rayleigh criterion `1.22·λ/D` while the modelled pupil's real first null is at `1.124·λ/D`. That constant gates `Resolvable` and scales every speckle floor and SNR in §4.1, so it is left alone deliberately rather than silently reconciled (§7.111).
   - **9c.** The profile's average over a pixel's azimuthal extent is exact within 6 px of the source and taken in its narrow-angle limit beyond, agreeing with a brute-force average over a real square pixel to **6.7×10⁻⁴ of peak intensity** across every plate scale the display produces (§7.111).
...
- Høg, E. et al. (2000). *A&A* 355, L27. The Tycho-2 catalogue (rendered star field).
- ESA SP-1200 Vol. 1, §1.3 (1997). Tycho VT/BT → Johnson V, B-V transformation.
